...
def wakeup_output():
    ...
    mod_keys_to_reset = [
        Key.LEFT_SHIFT, Key.RIGHT_SHIFT,                        # Relatively safe to press/release
        Key.LEFT_CTRL, Key.RIGHT_CTRL,                          # Relatively safe to press/release?
        # Key.LEFT_ALT, Key.RIGHT_ALT,                            # Alt causes menu focus stealing
        # Key.LEFT_META, Key.RIGHT_META,                          # Meta causes actions in many DEs
        # Key.CAPSLOCK, Key.NUMLOCK                               # Toggling LEDs probably not needed
    ]

    # # List of all typical keyboard keys, including modifiers
    # keys_to_reset = [
    #     # Modifier keys
    ...
    # all_keys_to_reset = modifier_keys + keys_to_reset
    ...
    total_keys = len(mod_keys_to_reset)
    chars_filled = 0

    def send_sync_event_with_dummy_dev():
        """Send a sync event with the dummy device"""
        sync_event = InputEvent(0, 0, ecodes.EV_SYN, 0, 0)
        on_event(sync_event, dummy_device)

    # Generate release events for each key
    for i, key in enumerate(mod_keys_to_reset):
        ...
        send_sync_event_with_dummy_dev()
